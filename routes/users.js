...
router.post('/', (req, res, next) => {
  ...
  User.create(req.body, (err, user) => {
    ...
    let mailOptions = {
      ...
      <a href="http://localhost:4000/users/varify-email?token=${user.emailToken}">Varify your account</a>
      `,
    };
...
router.get('/email/:email', async (req, res, next) => {
  ...
  let mailOptions = {
    ...
    to: otpData.email,
    subject:
      'One Time Password (OTP) for Password recovery process on Expense Tracker',
      ...
      Income.distinct('source', (err, incomes) => {
        ...
        Expense.distinct('category', (err, expenses) => {
          ...
          if (category && start_date && end_date) {
            ...
            let data = epn
              .filter((elm) => elm.date >= start_date && elm.date <= end_date)
          ...
          if (month) {
            let data = mixedArr
              .filter((elm) => elm.date.includes(month))
              .sort((a, b) => a.createdAt - b.createdAt);
